...
func artifactOutputContract(kind DraftKind) string {
	...
	case DraftSkills:
		lines = append(lines,
			...
			"  - MCP Input Schema: (emit a fenced JSON block containing a valid JSON Schema object for this skill's inputs. The schema is used to generate MCP-compatible tool definitions. Format:)",
			"    ```json",
			"    {\"type\": \"object\", \"properties\": {\"param_name\": {\"type\": \"string\", \"description\": \"what it is\"}}, \"required\": [\"param_name\"]}",
			"    ```",
			"    Every parameter from Inputs Required must appear in this schema. Use JSON Schema types: string, integer, number, boolean, array, object. This block is machine-parsed -- do not add comments or trailing commas.)",
			...
			"    2. When source material contains scripts, CLI tools, or executable commands, include the EXACT invocation command in the step (e.g., `python3 script.py -c config.txt input.txt --csv output.csv`). Agents execute these directly; omitting commands forces every agent to independently reconstruct the invocation, wasting tokens across the swarm.",
			"    3. Include decision branches: 'If X then Y. If X fails, Z.' Not every step branches, but every step where the source implies a condition must show both paths.",
			"    4. Include failure handling: what happens when this step cannot complete? Retry, reject, escalate, or mark UNKNOWN.",
			"    5. For steps that produce intermediate state (e.g., classified alert, correlated group), mark as CHECKPOINT -- the agent should persist this state before proceeding so recovery can resume from here.",
			"    6. End with the citation to the source that justifies this step.",
			...
			"  - Applicable Standards: (identify industry standards, frameworks, or compliance requirements that apply to the domain described in the source material. Examples: MITRE ATT&CK for security/threat hunting, OWASP for web application security, PCI-DSS for payment systems, HIPAA for healthcare data, SOX/Basel for finance, NIST CSF for cybersecurity programs. Map skill capabilities to specific standard identifiers where possible, e.g., MITRE technique IDs like T1003. If no recognized standard applies, omit this section rather than inventing one.)",
			"",
			"EXECUTABLE COMMAND RULES:",
			"- When the source material contains scripts, CLI tools, or documented command-line invocations, every Process step that uses a tool must include the exact command with arguments.",
			"- Copy the command syntax from the source (README, docs, code comments) rather than inventing a new invocation.",
			"- Include the full command: interpreter, script path, required flags, input file, and output file.",
			"- Example: instead of 'run the hash lookup against mimikatz hashes', write 'run `python3 hash2processarg.py -c config.txt hashset/hacking-tools/mimikatz.txt --csv mimikatz_hits.csv`'.",
			"- If the exact command is not documented but the script exists in the source, construct the invocation from the script's argparse/click/flag definitions and mark it as an implementation decision.",
			"- Agents consume these commands directly at runtime. Missing commands force every agent to independently reconstruct the invocation, multiplying token cost across the swarm.",
			"",
